...
namespace StoreAPI.Controllers
{
    public class UserController : Controller
    {
        ...
        public IActionResult PlaceOrder()
        {
            ...
            payments.ForEach(_ => { _.CardNumber = _.CardNumber.Substring(12); _.CVV = null; });
            User u = new User
            {
                ShoppingCarts = shoppingCarts,
                Addresses = db.Addresses.Where(_ => _.UserId == user.UserId).ToList(),
                Payments = payments,
                Password = RequestToken(user.Username)
            };
        ...
        public IActionResult PromoCode(string Code)
        {
            string token = HttpContext.Request.Query["token"].ToString();
            User user = Authorize(token);
            if (user == null)
                return new UnauthorizedResult();
            PromoCode promoCode = db.PromoCodes.FirstOrDefault(_ => _.Code == Code);
            if (promoCode == null)
        ...
        public IActionResult Checkout([FromBody] OrderDetail orderDetail)
        {
            ...
            {
                PromoCode promoCode = db.PromoCodes.FirstOrDefault(_ => _.Code == orderDetail.Code);
                if (promoCode == null)
                    return new BadRequestResult();
        ...
        public IActionResult OrderList()
        {
            ...
            orderDetails.ForEach(_ => { _.User = null; _.PromoCode = null; });
            orderDetails.Sort((x, y) => y.PlacedTime.CompareTo(x.PlacedTime));
        ...
        public List<ShoppingCart> CheckStock(User user)
        {
            ...
            db.SaveChanges();
            shoppingCarts.Sort((x, y) => y.AddTime.CompareTo(x.AddTime));
